...
function Karel(karelWorld) {
	...
	function getKarelImage(cornerSize) {
	...
	this.draw = function(c) {
		...
		var img = getKarelImage(cornerSize);
		
		if (typeof img == "undefined" || img == null) {
			alert('karel image loading fail!');
		}
		
		var karelX = karelWorld.getLeft() + xIndex * cornerSize;
		var karelY = karelWorld.getTop() + yIndex * cornerSize;
		
		//draw karel one pixel smaller on all sides
		c.drawImage(img,karelX+1, karelY+1, cornerSize-2, cornerSize-2);
	}
